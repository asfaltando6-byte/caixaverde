...
const CTA = ({text="QUERO PROTEGER O LUCRO DO MEU MERCADINHO"}:{text?:string}) => <a className="cta" href="#oferta">{text} <b>→</b></a>;
...
export default function Home(){return <main>
 ...
 <section className="section bonusSection"><div className="shell narrow"><div className="center bonusHead"><span className="tag">BÔNUS INCLUÍDOS</span><h2>Leve também <em>2 materiais extras</em></h2><p className="lead">Ferramentas complementares para começar a organizar o mercadinho com mais rapidez.</p></div><div className="bonusGrid">{bonusItems.map((bonus,i)=><article key={bonus.title}><div className="bonusPreview"><img src={bonus.image} alt={`Prévia do bônus ${bonus.title}`} width="910" height="1287" loading="lazy"/><b>BÔNUS {i+1}</b></div><div><small>INCLUÍDO SEM CUSTO EXTRA</small><h3>{bonus.title}</h3><p>{bonus.description}</p></div></article>)}</div></div></section>

 <section className="section offer" id="oferta"><div className="shell narrow"><div className="offerCard"><header><small>OFERTA ESPECIAL</small><h2>Comece hoje a enxergar onde o dinheiro do seu mercadinho está indo</h2></header><div className="offerMockup"><img src="/assets/product-mockup-v2.webp" alt="CaixaVerde: Manual dos 7 Ralos" width="1536" height="1024" loading="lazy"/></div><div className="offerBody"><div><h3>CaixaVerde</h3><p>Manual dos 7 Ralos que Fazem o Dinheiro Sumir</p>{["Manual completo em PDF","Diagnóstico com pontuação","Plano de ação de 7 dias","Checklists de aplicação","Folhas práticas para imprimir","2 bônus práticos"].map(x=><span className="offerItem" key={x}><Check/>{x}</span>)}</div><div className="buy"><small>ACESSO AO MATERIAL COMPLETO</small><div className="price"><span>por apenas</span><strong><sup>R$</sup>37<sup>,00</sup></strong><span>pagamento único</span></div><a className="cta" href="#">QUERO O MANUAL AGORA <b>→</b></a><p>🔒 <strong>Compra segura</strong><small>Pagamento em ambiente protegido</small></p></div></div></div></div></section>
